...
각 병사는 특정한 값의 전투력을 보유하고 있으며, 병사를 배치할 때는 전투력이 높은 병사가 앞쪽에 오도록 내림차순으로 배치를 하고자 합니다.
다시 말해 앞쪽에 있는 병사의 전투력이 항상 뒤쪽에 있는 병사보다 높아야 합니다.
또한 배치 과정에서는 특정한 위치에 있는 병사를 열외시키는 방법을 이용합니다.
그러면서도 남아있는 병사의 수가 최대라 되도록 하고 싶습니다.
예를 들어, N = 7일때 나열된 병사들의 전투력이 다음과 같다로 가정하겠습니다.
...
'''
점화식: 모든 O <= j <= i에 대하여, D[i] = max(D[i], D[j] + 1) if array[j] < array[i]
'''

n = int(input())
array = list(map(int, input().split()))
# 순서를 뒤집어 '기징 긴 증가하는 부분 수열' 문제로 변환
array.reverse()
